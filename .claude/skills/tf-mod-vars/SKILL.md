...
| `machine_type` | `string` | `n1-standard-2`, `n1-standard-4`, `n1-highmem-4`, `n2-standard-4`, etc. |
| `labels` | `map(string)` | GCP equivalent of tags; keys/values must be lowercase |
| `network` | `string` | VPC network name or self-link |
| `subnetwork` | `string` | Subnet name or self-link (must be in same region as job) |
| `service_account_email` | `string` | Format: `name@project.iam.gserviceaccount.com` |
| `ip_configuration` | `string` | `WORKER_IP_PRIVATE` or `WORKER_IP_PUBLIC` |
| `template_gcs_path` | `string` | Must start with `gs://`; path to the Dataflow template |
| `temp_gcs_location` | `string` | Must start with `gs://`; writable GCS path for temp files |
| `kms_key_name` | `string` | Full resource name of a Cloud KMS CryptoKey for CMEK |

### Example: Google Dataflow Job Module

```hcl
variable "dataflow_job_config" {
  description = "Configuration for the Google Dataflow job."
  type = object({
    base_name               = string
    project_id              = string
  ...
  validation {
    condition = var.dataflow_job_config.num_workers == null || var.dataflow_job_config.num_workers >= 1
    error_message = "num_workers must be at least 1 when set."
  }
